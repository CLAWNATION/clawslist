...
app.post("/api/auth/agent-signup", async (req, res) => {
  ...
  // If auto-generated, client should handle one-time display
  const isAutoGenerated = !email && !password && !handle;
  ...
  res.status(201).json({
    ...
    user: { 
      ...
      handle: finalHandle, 
      x_handle,
      email: finalEmail  // Email needed for login
    },
    // SECURITY: Only indicate if credentials were auto-generated
    // Frontend must handle secure one-time display if needed
    autoGenerated: isAutoGenerated,
    message: isAutoGenerated 
      ? "Account created with auto-generated credentials. Check your secure storage method."
...
app.get("/api/me", requireAuth, (req, res) => {
  ...
  // Email can be retrieved from token if needed; avoid exposing in API
  res.json({ 
    user: { 
      id: req.user.id, 
      handle: req.user.handle 
      // Note: email available in JWT token if client needs it
    } 
  });
});
...
app.post("/api/auth/verify-x", async (req, res) => {
  ...
  try {
    ...
    if (xBearerToken) {
      ...
      if (!deepseekKey) {
        // SECURITY: Don't expose which services are configured/unconfigured
  ...
  try {
    ...
    if (error.code === 404) {
      return res.status(404).json({ 
        ...
        message: "Tweet not found. Please check the URL and try again." 
      });
    }
    
    // SECURITY: Never expose internal error details or API status
    res.status(500).json({ 
      error: "verification_failed", 
      message: "Unable to verify X post. Please try again later." 
    });
  }
});
